...
async function findDomainGist(token: string): Promise<string | null> {
  ...
  const allGists: Array<{ id: string; description: string; files: Record<string, unknown>; updated_at: string }> = [];
  let page = 1;
  const perPage = 30;
...
export const onRequest = async (context: CloudflareContext) => {
  ...
  try {
    ...
    if (action === 'export') {
      ...
      let targetGistId: string | undefined = gistId;
      if (!targetGistId) {
        targetGistId = (await findDomainGist(token)) ?? undefined;
      }
