...
function resolveWxpushSendUrl(baseOrFull: string): string {
...
async function sendWxpushNotify(title: string, content: string, url: string, token: string) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`wxpush请求失败: ${res.status} ${res.statusText}${errorText ? ` - ${errorText}` : ''}`);
  }
  ...
  if (method === 'POST') {
    try {
      ...
      if (body.domains) {
        ...
        if (env.WXPUSH_URL && env.API_TOKEN) {
          envMethods.push('wxpush');
        }
        ...
        for (const method of notifyMethods) {
          try {
            if (method === 'telegram') {
              ...
              const baseOrFull = env.WXPUSH_URL;
              const token = env.API_TOKEN;
              if (!baseOrFull || !token) {
                const error = '未配置 WXPUSH_URL 或 API_TOKEN';
                await logNotificationDetail(env, 'WXPUSH_ERROR', error, 'error', undefined, 'wxpush', error);
                throw new Error(error);
              }
              ...
              await sendWxpushNotify('域名到期提醒', content, url, token);

              results.push({ method: 'wxpush', ok: true });
