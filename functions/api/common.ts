...
export async function initializeDatabase(env: any) {
  try {
    ...
    `).run();
    try {
      await env.DB.prepare(`ALTER TABLE notification_settings ADD COLUMN bg_image_url TEXT`).run();
    } catch {}
    try {
      await env.DB.prepare(`ALTER TABLE notification_settings ADD COLUMN carousel_interval INT`).run();
    } catch {}
    try {
      await env.DB.prepare(`ALTER TABLE notification_settings ADD COLUMN carousel_enabled TEXT`).run();
    } catch {}
    await env.DB.prepare(`
