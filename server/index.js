...
/** `WXPUSH_URL` 可为服务根地址或已含 `/wxsend` 的完整地址 */
function resolveWxpushSendUrl(baseOrFull) {
  const s = String(baseOrFull).trim().replace(/\/+$/, '');
  if (!s) return s;
  if (/\/wxsend$/i.test(s)) return s;
  return `${s}/wxsend`;
}

async function sendWxpushNotify(title, content, url, token) {
  ...
  if (!r.ok) {
    ...
    throw new Error(`wxpush请求失败: ${r.status} ${r.statusText}${errorText ? ` - ${errorText}` : ''}`);
  }
...
app.post('/api/notify', async (req, res) => {
  try {
    ...
    if(body.domains){
      ...
      if(process.env.WXPUSH_URL && process.env.API_TOKEN) notifyMethods.push('wxpush');
      ...
      let warningDays=15;
      try{const r=await pool.query('SELECT warning_days FROM notification_settings LIMIT 1'); if(r.rows.length) warningDays=parseInt(r.rows[0].warning_days)||15;}catch(e){
        void e;
      }
      ...
      for(const method of notifyMethods){
        try{
          ...
          }else if(method==='wxpush'){
            const baseOrFull=process.env.WXPUSH_URL, token=process.env.API_TOKEN; if(!baseOrFull||!token)throw new Error('未配置 WXPUSH_URL 或 API_TOKEN');
            const url=resolveWxpushSendUrl(baseOrFull);
            let content=`以下域名将在${warningDays}天内到期：\n\n`;
            expiringDomains.forEach(d=>{const left=getDaysUntilExpiry(d.expire_date); content+=`域名: ${d.domain}\n注册商: ${d.registrar}\n到期时间: ${d.expire_date}\n剩余天数: ${left}天\n\n`; }); content+='请及时续费以避免域名过期！';
            await sendWxpushNotify('域名到期提醒', content, url, token); results.push({method:'wxpush',ok:true});
...
app.post('/api/webdav', async (req, res) => {
  try {
    ...
    if (action === 'backup') {
      ...
      try{ await fetch(domainFolderUrl, {method:'MKCOL',headers:{'Authorization': `Basic ${auth}`}}); }catch(e){
      void e;
      }
