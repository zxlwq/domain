...
async function sendWxpushNotify(title, content, url, token) {
  ...
  if (!resp.ok) {
    ...
    throw new Error(`wxpush请求失败: ${resp.status} ${resp.statusText}${errorText ? ` - ${errorText}` : ''}`);
  }
  ...
  if (method === 'POST') {
    try {
      ...
      if (body.domains) {
        ...
        if (process.env.WXPUSH_URL && process.env.API_TOKEN) { envMethods.push('wxpush'); }
        ...
        for (const method of notifyMethods) {
          try {
            if (method === 'telegram') {
              ...
              const baseOrFull = process.env.WXPUSH_URL;
              const token = process.env.API_TOKEN;
              if (!baseOrFull || !token) throw new Error('未配置 WXPUSH_URL 或 API_TOKEN');
